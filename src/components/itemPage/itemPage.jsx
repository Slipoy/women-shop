...
import styleBtn from "../ItemsTest/items.module.css"
import star from    '../../assets/star/Vector (3).png'

import BasketSrc from '../../assets/header/basket.png'
    ...
    return(
        <div className={style.itemPage}>
            <div className={style.pagePath}>
                ...
                {itemInfo ? <span>/{itemInfo.map(item => item.name)}</span> : <span></span>}
            </div>
            <h2>{itemInfo.map(item => item.name)} <span className={style.artItem}>Арт {itemId}</span></h2>
            ...
            <div className={style.bodyItemPage}>
                ...
                <div className={style.descriptionItem}>
                    <span className={style.priceItem}>{itemInfo.map(item=> item.price)} грн</span>
                    ...
                    <div className={style.sizeTitle}>
                        ...
                        <span className={`${style.tableSize} ${style.upperLineText}`}>Таблица размеров</span>
...
<rect fill="#514a7e"/> </svg>
                    ...
                    <div className={style.reviews}>
                        <div className={style.stars}>
                            {initStars(itemInfo.map(item => item.stars)).map((item, index)=> <img key={index} src={star} alt=""/>)}
                        </div>
                        <div className={style.reviewsBtn}>
                            <span className={`${style.upperLineText} ${style.allReviews}`}>Смотреть все отзывы</span>
                            <span className={`${style.upperLineText} ${style.addReview}`}>Добавить отзыв</span>
                        </div>
                    </div>
...
export default compose(connect(mapStateToProps, {setAllDataCatalog,setCatalogItems}),withRouter)(ItemPage)